...
public class GKTest {
	...
	void testGK() {
		...
		System.out.println();
		System.out.println("Test GK simple");

		double quantile = gkComputer.quantile(phi);		// expected 77 (computed with libreoffice calc)
		System.out.printf("Avg quantile %f: %f%n", phi, quantile);

	}

	void testGKAddObservations() {
		double phi = 0.75;
		GKComputer gkComputer = new GKComputer(0.01);
		gkComputer.insertMany(firstList);

		System.out.println();
		System.out.println("Test GK add observations");

		double quantile = gkComputer.quantile(phi);		// expected 77 (computed with libreoffice calc)
		System.out.printf("Avg quantile %f (first list): %f%n", phi, quantile);

		gkComputer.insertMany(secondList);
		quantile = gkComputer.quantile(phi);			// expected 75 (computed with libreoffice calc)
		System.out.printf("Avg quantile %f (full list): %f%n", phi, quantile);
	...
	void testGKRestore() {
		...
		System.out.println();
		System.out.println("Test GK restore");

		// print quantile
		double quantile = gkComputer.quantile(phi);			// expected 75 (computed with libreoffice calc)
		System.out.printf("Original quantile %f (full list): %f%n", phi, quantile);
		...
		System.out.printf("Restored quantile %f (full list): %f%n", phi, quantile);
	...
	void testGKMerge() {
		...
		System.out.println();
		System.out.println("Test GK merge");
		...
		System.out.printf("Avg quantile %f (merged models): %f%n", phi, quantile);
	...
	public void testGKBig() {
		...
		System.out.println();
		System.out.println("Test GK big");

		for (int i = 0; i < nbAttempts; i++) {
			gkModel.insertAndCompress(random.nextInt(1000000));

			if (i % 5000 == 0) {
				quantile = gkModel.quantile(phi);
				System.out.printf("Intermediate quantile %f (iter %d): %f\n", phi, i, quantile);
			}
		}
	}
